...
export const authOptions = {
  ...
  callbacks: {
    async signIn({ user, account }) {
      ...
      // username/password ปกติ (ไม่ใช่ path admin ที่ไม่มีแถวจริงในตาราง users) —
      // ของเดิมอัปเดต last_login ให้แค่ตอน login ด้วย Google เท่านั้น ทำให้คนที่ล็อกอินด้วย
      // username/password อย่างเดียวมี last_login ค้างอยู่ที่วันสมัครตลอดไป ไม่เคยขยับเลย
      if (account?.provider === "credentials" && user.email !== "__admin__") {
        await supabaseAdmin
          .from("users")
