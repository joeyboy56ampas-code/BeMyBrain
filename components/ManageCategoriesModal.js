...
function IconGrid({ selected, onPick }) {
  return (
    <div style={{ background: INK, border: `1px solid ${INK_LINE}` }} className="flex flex-wrap gap-1.5 rounded-lg p-2 mt-1.5">
      {CATEGORY_ICON_KEYS.map((key) => {
        ...
        return (
          <button
            key={key}
            onClick={() => onPick(key)}
            style={{ background: isSelected ? GOLD : INK_SOFT, border: `1px solid ${isSelected ? GOLD : INK_LINE}` }}
            className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0"
          >
            <Icon size={15} style={{ color: isSelected ? INK : TEXT_MUTED }} />
          </button>
        );
      })}
    </div>
...
export default function ManageCategoriesModal({ open, onClose, categories, entries, t, onAdd, onRename, onChangeIcon, onDelete }) {
  const [newName, setNewName] = useState("");
  const [newIcon, setNewIcon] = useState("Sparkles");
  const [editingId, setEditingId] = useState(null);
  const [editValue, setEditValue] = useState("");
  const [pickerFor, setPickerFor] = useState(null); // null | "new" | category id
              ...
              return (
                <div key={c.id}>
                  ...
                  {pickerOpen && (
                    <IconGrid
                      selected={c.icon}
                      onPick={(key) => { onChangeIcon(c.id, key); setPickerFor(null); }}
                    />
                  )}
                </div>
              );
          ...
          <div style={{ borderTop: `1px solid ${INK_LINE}` }} className="px-5 py-4 flex flex-col gap-3">
            <div className="flex gap-2">
              <button
                ...
                title={t("choose_icon")}
              >
                {(() => { const NewIcon = ICONS[newIcon] || ICONS.Sparkles; return <NewIcon size={15} style={{ color: GOLD }} />; })()}
              </button>
              <input
                value={newName}
            ...
            {pickerFor === "new" && <IconGrid selected={newIcon} onPick={(key) => setNewIcon(key)} />}
            <button
              onClick={onClose}
